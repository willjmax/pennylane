...
class TestDecomposition:
    ...
    @pytest.mark.parametrize(
        "s_wires, d_wires, weights, ref_gates", test_data_decomposition
    )
...
def circuit_decomposed(weights):
    ...
    for weight in weights:
        qml.FermionicDoubleExcitation(weight[1], wires1=[0, 1], wires2=[2, 3])
        qml.FermionicSingleExcitation(weight[0], wires=[0, 1])
    return qml.expval(qml.PauliZ(0))
...
class TestInterfaces:
    ...
    def test_autograd(self, tol):
        ...
        weights = pnp.array(np.random.random(size=(1,2)), requires_grad=True)
    ...
    def test_tf(self, tol):
        ...
        weights = tf.Variable(np.random.random(size=(1,2)))
        dev = qml.device("default.qubit", wires=4)
    ...
    def test_torch(self, tol):
        ...
        weights = torch.tensor(np.random.random(size=(1,2)), requires_grad=True)
